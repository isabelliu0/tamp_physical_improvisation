...
class GoalConditionedWrapper(gym.Wrapper):
    ...
    def __init__(
        ...
        # Create Dict observation space required for HER
        assert isinstance(
            env.observation_space, gym.spaces.Box
        ), "Only Box obs spaces supported"
    ...
    def configure_training(self, train_data: GoalConditionedTrainingData) -> None:
        ...
        print(
            f"Updated {len(self.node_states)} node states and {len(self.valid_shortcuts)} valid shortcuts from training data"  # pylint: disable=line-too-long
        )
    ...
    def compute_reward(
        ...
        rewards = np.where(
            distance < self.success_threshold, self.success_reward, self.step_penalty
        )
        if isinstance(info, list):
            for i, info_dict in enumerate(info):
                if i < len(distance):
                    info_dict["is_success"] = bool(distance[i] < self.success_threshold)
