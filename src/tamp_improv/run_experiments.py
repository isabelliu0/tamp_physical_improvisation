...
def run_pybullet_multi_seed_experiment(
    system_cls: type,
    use_context_wrapper: bool,
    seeds: list[int],
    episodes_per_scenario: int,
):
    """Run the PyBullet experiment with multiple seeds and return aggregated
    results."""
    all_metrics = []
    ...
    for seed in seeds:
        print(f"\n\n=== Running PyBullet experiment with seed {seed} ===")
...
def run_pybullet_experiment(episodes_per_scenario: int):
    """Run the PyBullet experiment with multiple seeds."""
    print("\n=== Running PyBullet Multi-Policy RL Experiment ===")
    print(f"Episodes per scenario: {episodes_per_scenario}")

    results = run_pybullet_multi_seed_experiment(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["blocks2d", "pybullet"],
        help="System to use for experiments (blocks2d or pybullet)",
    )
    parser.add_argument(
    ...
    else:  # pybullet clear-and-place
        run_pybullet_experiment(episodes_per_scenario=args.episodes)
